...
def user_ratings(context, course, review, icon_height=app_settings.STAR_RATINGS_STAR_HEIGHT, icon_width=app_settings.STAR_RATINGS_STAR_WIDTH, read_only=False):
    request = context.get('request')

    if request is None:
        raise Exception('Make sure you have "django.core.context_processors.request" in "TEMPLATE_CONTEXT_PROCESSORS"')

    rating = Rating.objects.for_instance(course)
    user = request.user.is_authenticated() and request.user or None

    if request.user.is_authenticated() or app_settings.STAR_RATINGS_ANONYMOUS:
        user_rating = UserRating.objects.for_instance_by_user(course, user=user)
    else:
        user_rating = None
    ...
    user_star = [i for i in range(1, review+1)]#int(rating.average)+1)]#app_settings.STAR_RATINGS_RANGE + 1)]
    ...
    return {
        ...
        'star_count': user_star,#app_settings.STAR_RATINGS_RANGE,
